...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $req = $pdo->prepare("SELECT * FROM document_requirements WHERE id = ? AND audience IN ('staff','all') AND is_active = 1");
    $req->execute([$requirementId]);
    $requirement = $req->fetch();
    
    if (!$requirement) {
        flash('error', 'Invalid document requirement.');
        redirect('staff/documents.php');
    }
...
$requirements = $pdo->query("SELECT * FROM document_requirements WHERE audience IN ('staff','all') AND is_active = 1 ORDER BY name")->fetchAll();
