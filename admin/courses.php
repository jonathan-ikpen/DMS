...
<div style="display: flex; gap: 16px; margin-bottom: 24px;">
    <input type="text" id="courseSearch" placeholder="Search by code or title..." style="flex: 1;" onkeyup="filterCourses()">
    <select id="levelFilter" style="width: 200px;" onchange="filterCourses()">
...
function filterCourses() {
    const search = document.getElementById('courseSearch').value.toLowerCase().replace(/\s+/g, '');
    const level = document.getElementById('levelFilter').value;
    const rows = document.querySelectorAll('.course-row');
    
    rows.forEach(row => {
        const rowLevel = row.getAttribute('data-level');
        const code = row.querySelector('.course-code').innerText.toLowerCase().replace(/\s+/g, '');
        const title = row.querySelector('.course-title').innerText.toLowerCase().replace(/\s+/g, '');
        
        const matchesSearch = code.includes(search) || title.includes(search);
